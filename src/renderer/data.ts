...
COMPONENT_REGISTRY.set("table", (node: ComponentNode, _ctx: RenderContext): RenderResult => {
  ...
  // Headers come from pipe-separated segments: text is the first column,
  // fields[] are the remaining columns (e.g. "table Deal | Value | Stage"
  // → text="Deal", fields=["Value","Stage"]).
  const headers: string[] = []
  if (node.text) {
    headers.push(node.text, ...node.fields.filter(Boolean))
  } else if (node.fields.length > 0) {
    // Legacy comma-separated fallback in fields[0]
    headers.push(
      ...node.fields[0]!.split(",").map(h => h.trim()).filter(Boolean),
    )
  }
  ...
  if (node.children.length > 0) {
    // Children are data rows — each child's text + fields are cell values
    // (parsed from pipe-separated DSL lines like "Acme Corp | $125,000 | Proposal")
    for (const child of node.children) {
      const tr = document.createElement("tr")
      const cells = [child.text, ...child.fields].filter(
        (_v, i) => i === 0 || true,
      )
