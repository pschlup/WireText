...
describe("data", () => {
  ...
  it("table with 4 pipe-separated columns inside card renders all headers", () => {
    // Simulates: table Deal | Value | Stage | Close Date
    // Parser produces: text="Deal", fields=["Value","Stage","Close Date"]
    const tableNode = makeNode("table", "Deal", {
      fields: ["Value", "Stage", "Close Date"],
    })
    // Nest inside a card to match the bug scenario
    const cardNode = makeNode("card", "Hot Deals", { children: [tableNode] })
    const { element } = render(cardNode)
    const headers = element.querySelectorAll("th")
    expect(headers).toHaveLength(4)
    expect(headers[0]?.textContent).toBe("Deal")
    expect(headers[1]?.textContent).toBe("Value")
    expect(headers[2]?.textContent).toBe("Stage")
    expect(headers[3]?.textContent).toBe("Close Date")
  })

  it("table with data rows inside card renders correct cell values", () => {
    // Simulates a table with children (indented data rows)
    const row1 = makeNode("item", "Acme Corp Expansion", {
      fields: ["$125,000", "Proposal", "Mar 28"],
    })
    const row2 = makeNode("item", "Global Tech Renewal", {
      fields: ["$89,000", "Negotiation", "Apr 5"],
    })
    const tableNode = makeNode("table", "Deal", {
      fields: ["Value", "Stage", "Close Date"],
      children: [row1, row2],
    ...
    expect(firstRowCells?.[0]?.textContent).toBe("Acme Corp Expansion")
    expect(firstRowCells?.[1]?.textContent).toBe("$125,000")
    expect(firstRowCells?.[2]?.textContent).toBe("Proposal")
    expect(firstRowCells?.[3]?.textContent).toBe("Mar 28")
  })
...
describe("compounds", () => {
  ...
  it("data-table with pipe-separated headers and child rows renders visible table", () => {
    // Simulates data-table used without slot syntax: headers from text+fields,
    // rows from children — the pattern that was producing no output
    const row1 = makeNode("item", "Alice Johnson", {
      fields: ["alice@acme.com", "Admin", "Active", "Mar 12", "$4,200"],
    })
    const row2 = makeNode("item", "Bob Martinez", {
      fields: ["bob@acme.com", "Editor", "Pending", "Mar 11", "$1,850"],
    })
    const node = makeNode("data-table", "Name", {
      fields: ["Email", "Role", "Status", "Date", "Revenue"],
      children: [row1, row2],
    })
    const { element } = render(node)
    const table = element.querySelector("table.wt-data-table")
    ...
    expect(ths?.[5]?.textContent).toBe("Revenue")
    const rows = table?.querySelectorAll("tbody tr")
    expect(rows?.length).toBe(2)
    const firstRowCells = rows?.[0]?.querySelectorAll("td")
    expect(firstRowCells?.[0]?.textContent).toBe("Alice Johnson")
    expect(firstRowCells?.[1]?.textContent).toBe("alice@acme.com")
  })

  it("data-table child row data preserves all fields", () => {
    const row = makeNode("item", "Carol Chen", {
      fields: ["carol@acme.com", "Viewer"],
    })
    const node = makeNode("data-table", "Name", {
      fields: ["Email", "Role"],
      children: [row],
    ...
    expect(tds[0]?.textContent).toBe("Carol Chen")
    expect(tds[1]?.textContent).toBe("carol@acme.com")
    expect(tds[2]?.textContent).toBe("Viewer")
  })
